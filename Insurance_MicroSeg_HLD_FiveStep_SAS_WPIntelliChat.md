...
sequenceDiagram
  ...
  SCOR-->>WS: cluster_id, risk_score, top_features

  WS->>ID: Evaluate tariff guardrails & underwriting rules
  ID-->>WS: Recommended action (below-ceiling / at-ceiling)

  WS->>WPIC: Request evidence pack (context filters)
  WPIC->>WPIC: Retrieve chunks & compose narrative
  WPIC-->>WS: Narrative + citations (PDF/HTML)

  WS-->>UW: Show score, recommendation, evidence
  UW-->>WS: Finalize quote; attach evidence for compliance
```
